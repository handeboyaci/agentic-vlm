...
def download_pdbbind():
  ...
  try:
    dataset = MoleculeNet(
      root=data_dir, name="PDBbind", pre_transform=None, pre_filter=None
    )
    print("Successfully downloaded PDBbind dataset.")
    ...
    print(
      "Attempting to use ESOL as a surrogate if PDBbind fails (PDBbind sometimes requires login or has changed URLs)."
    )
    try:
      dataset = MoleculeNet(root=data_dir, name="ESOL")
      print("Successfully downloaded ESOL dataset as surrogate.")
      print(f"Number of graphs: {len(dataset)}")
